...
/**
 * The SQLParser class provides a set of static methods for parsing and matching SQL statements against specific patterns.
 * These methods are designed to identify various supported SQL constructs, primarily focusing on VoltDB-specific
 * syntax structures, enabling developers to process or validate statements effectively.
 *
 * Each method typically accepts a SQL statement as input and returns a Matcher object if the input matches
 * a predefined pattern. The available methods cover a wide range of functionalities, including detecting
 * creation, alteration, migration, replication, or deletion of database objects like tables, roles, streams, procedures,
 * or functions.
 *
 * The pattern-matching methods in this class are particularly useful in scenarios involving:
 * - Syntax validation for SQL DDL or DML statements.
 * - SQL statement-specific feature extraction.
 * - Parsing and processing of complex SQL-based workflows.
 *
 * This class encapsulates the parsing logic for recognized SQL statements, making it integral for tools dealing with
 * SQL-related operations on databases, especially VoltDB.
 */
public class SQLParser extends SQLPatternFactory
{
    ...
    private static final Pattern DashBatchToken = Pattern.compile(
            ...
            "-{1,2}batch", // -batch option, whitespace terminated
            Pattern.CASE_INSENSITIVE);

    private static final Pattern DashInlineBatchToken = Pattern.compile(
            "\\s+" +             // required preceding whitespace
            "-{1,2}inlinebatch", // -inlinebatch option, whitespace terminated
            Pattern.CASE_INSENSITIVE);
    ...
    /**
     * Matches the given statement against the pattern for creating a task.
     *
     * @param statement the input string to be matched against the create task pattern
     * @return a Matcher object that can be used to determine if the input matches the pattern and to extract matched parts
     */
    ...
    /**
     * Matches the given statement against the pattern for dropping a task.
     *
     * @param statement the input string to be matched for a drop task command
     * @return a Matcher instance if the statement matches the drop task pattern, otherwise an empty Matcher
     */
    ...
    public static class FileInfo {
        ...
        private final int m_fileSequence;
        private final String m_delimiter;
        private static FileInfo m_oneForSystemIn = null; // Create on demand.

        public FileInfo(String path) {
            this(null, FileOption.PLAIN, path, 0);
        }

        public FileInfo(String path, boolean batch) {
            this(null, batch ? FileOption.BATCH : FileOption.PLAIN, path, 0);
        }

        private FileInfo(FileInfo context, FileOption option, String filenameOrDelimiter, int fileSeq) {
            m_context = context;
            m_option = option;
            switch (option) {
            case PLAIN:
     ...
     * @param parentContext   optional parent file context for better diagnostics.
     * @param statement  statement to parse
    ...
    public static List<FileInfo> parseFileStatement(FileInfo parentContext, String statement)
    {
        ...
        if (inlineBatchMatcher.lookingAt()) {
            ...
            if (delimiterMatcher.matches()) {
                ...
                filesInfo.add(new FileInfo(parentContext, FileOption.INLINEBATCH, delimiter, 0));
                return filesInfo;
            }
        ...
        // resulting FileInfo objects are numbered in sequence for benefit of sqlcmd
        int sequence = 0;
        for (String filename : filenames) {
            ...
            if (filenameMatcher.matches()) {
                ...
                filesInfo.add(new FileInfo(parentContext, option, filename, ++sequence));
            }
        }
    ...
    public static long hexDigitsToLong(String hexDigits) throws Exception {
        ...
        if (hexDigits.length() > 16) {
            throw new Exception("Too many hexadecimal digits for BIGINT value");
        }

        if (hexDigits.length() == 0) {
            throw new Exception("Zero hexadecimal digits is invalid for BIGINT value");
        }
    ...
    /**
     * Parse EXECUTE procedure call with delayed loading of procedures
     * @param statement     statement to parse
     * @param procSupplier  provides map of procedures to parameter signature maps
     * @return              results object or NULL if statement wasn't recognized
     * @throws Exception
     */
    public static ExecuteCallResults parseExecuteCall(
            String statement,
            Supplier<Map<String,Map<Integer, List<String>>>> procSupplier) throws Exception
    {
        assert(procSupplier != null);
        return parseExecuteCallInternal(statement, procSupplier);
    }

    /**
     * Parse EXECUTE procedure call without looking up parameter types.
     ...
     * Calls for procedures map only after recognizing the statement,
     * thus providing for delayed loading. Also supports short-
     * circuiting procedure lookup for testing.
     * @param statement     statement to parse
     * @param procSupplier  provides map of procedures to parameter signature maps
     * @return              results object or NULL if statement wasn't recognized
     * @throws Exception
    ...
    private static ExecuteCallResults parseExecuteCallInternal(
            String statement,
            Supplier<Map<String,Map<Integer, List<String>>>> procSupplier
            ) throws Exception
    {
        ...
        if (OneWhitespace.matcher(commandWordTerminator).matches() ||
            ...
            if (procSupplier == null) {
                return results;
            }

            Map<String,Map<Integer, List<String>>> procedures = procSupplier.get();
            ...
            if (results.paramTypes == null || results.params.size() != results.paramTypes.size()) {
                ...
                throw new Exception("Invalid parameter count for procedure: %s (expected: %s received: %d)",
        ...
        if (commandWordTerminator.equals(";")) {
            ...
            throw new Exception("Incomplete EXECUTE command. EXECUTE requires a procedure name argument.");
        }
        throw new Exception("Invalid EXECUTE command. unexpected input: '" + commandWordTerminator + "'.");
    }
    ...
    /**
     * Parses a given SQL statement to identify and extract the portion of the
     * statement following the "EXPLAIN JSON" preamble.
     *
     * @param statement the SQL statement to parse
     * @return the substring of the statement following the "EXPLAIN JSON" preamble,
     *         or null if the preamble is not found
     */
    ...
    /**
     * Parses a given SQL statement to determine if it is an "EXPLAIN CATALOG" call.
     *
     * @param statement the SQL statement to parse and check for the "EXPLAIN CATALOG" pattern
     * @return true if the statement matches the "EXPLAIN CATALOG" pattern; false otherwise
     */
    ...
    /**
     * Parses an SQL statement to extract the procedure call after validating that it matches
     * the expected "EXPLAIN PROCEDURE" prefix.
     *
     * @param statement the SQL statement to be parsed. It should contain an "EXPLAIN PROCEDURE"
     *                  preamble followed by the procedure name.
     * @return the trimmed procedure name if the statement matches the expected format;
     *         returns null if the statement does not match.
     */
